        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-8">
          {pillars.map((pillar) => (
            <motion.div 
              ...
              viewport={{ once: true }}
              transition={{ delay: pillar.delay, duration: 0.8, ease: [0.16, 1, 0.3, 1] }}
              whileTap={{ scale: 0.95 }}
...
export const About = () => {
  const team = [
    ...
    },
    {
      ...
      role: "FOUNDER & AGENTIC CODING",
      bio: "Cofundador e especialista em Agentic Coding e IA. Transforma ideias em produtos reais com desenvolvimento ágil de sites, sistemas e soluções digitais sob medida.",
            ...
            >
              <div className="aspect-[4/5] bg-white/5 rounded-[3rem] md:rounded-[4rem] overflow-hidden border border-white/10 group relative mb-8 md:mb-12">
                ...
                <img 
                  ...
                  className={`w-full h-full object-cover ${member.position || 'object-center'} grayscale group-hover:grayscale-0 group-active:grayscale-0 transition-all duration-700 group-hover:scale-105 group-active:scale-105`}
                />
                {member.highlight && (
              ...
              <p className="text-violet-400 font-bold uppercase tracking-widest mb-6 text-[10px] md:text-sm">{member.role}</p>
              <p className="text-gray-400 text-sm md:text-base leading-relaxed mb-8 md:mb-10 whitespace-pre-line">{member.bio}</p>
